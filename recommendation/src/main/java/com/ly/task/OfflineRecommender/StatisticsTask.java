...
//        historyHotProducts(dataSet, env);
//        recentHotProducts(dataSet);
    ...
    public static void historyHotProducts() throws Exception {
        Table table2 = benv.sqlQuery("SELECT productId, COUNT(productId) as hot FROM rating group by productId order by hot desc");
    ...
    public static void goodProducts() throws Exception {
        Table table = benv.sqlQuery("select productId, avg(score) as avgScore from rating group by productId order by avgScore desc");
